...
var Mock = function(structure) {
  ...
  WriteStream.prototype.open = function() {
    var err, dirPointer;
    this.pointer = getPointer(this.path, structure);

    if (this.pointer && this.pointer instanceof File) {
      this.emit('open');
    } else if (this.pointer && typeof this.pointer === 'object') {
      this.close();
      err = new Error('Illegal operation on directory');
      err.code = 'EISDIR';
      
      this.emit('error', err);
    } else {
      dirPointer = getPointer(path.dirname(this.path), structure);
      if (dirPointer) {
        baseName = path.basename(this.path);
        dirPointer[baseName] = new File(Date.now(), '');
        this.pointer = getPointer(this.path, structure);
        this.emit('open');
      } else {
        this.close();
        err = new Error('No such file ' + this.path);
        err.errno = 34;
        err.code = 'ENOENT';
        err.path = this.path;
  ...
  WriteStream.prototype._write = function(data, encoding, cb) {
    ...
    this.pointer.content += data.toString();
    self._touchFile(this.path, Date.now());
    cb();
  };
